...
SWEP.Category               = "[NSN] Star Wars" -- The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
SWEP.Manufacturer = nil -- Gun Manufactrer (e.g. Hoeckler and Koch )
SWEP.Author             = "Ace" -- Author Tooltip
SWEP.Contact                = "https://discord.gg/GAd3Hsa" -- Contact Info Tooltip
SWEP.Purpose                = "Burn" -- Purpose Tooltip
SWEP.Instructions               = "Click" -- Instructions Tooltip
SWEP.Spawnable              = true -- Can you, as a normal user, spawn this?
SWEP.AdminSpawnable         = true -- Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair          = true      -- Draw the crosshair?
SWEP.DrawCrosshairIS = false -- Draw the crosshair in ironsights?
SWEP.PrintName              = "BT X-42 Republic Flamethrower"       -- Weapon name (Shown on HUD)
...
-- [[WEAPON HANDLING]] --
SWEP.Primary.Sound = Sound("weapons/weapon_nsn_flamethrower/wpn_flamethrower_shoot.mp3") -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.LoopSound = "weapons/weapon_nsn_flamethrower/wpn_flamethrower_lp.wav"	-- Looped fire sound, unsilenced
SWEP.Primary.LoopSoundSilenced = nil -- Looped fire sound, silenced
SWEP.Primary.LoopSoundTail = "weapons/weapon_nsn_flamethrower/wpn_flamethrower_off.mp3" -- Loop end/tail sound, unsilenced
...
SWEP.Primary.Sound_World = "weapons/weapon_nsn_flamethrower/wpn_flamethrower_shoot.mp3" -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound_World = nil -- This is the sound of the weapon, when silenced.

SWEP.Primary.LoopSound_World = "weapons/weapon_nsn_flamethrower/wpn_flamethrower_lp.wav" -- Looped fire sound, unsilenced
SWEP.Primary.LoopSoundSilenced_World = nil -- Looped fire sound, silenced
SWEP.Primary.LoopSoundTail_World = "weapons/weapon_nsn_flamethrower/wpn_flamethrower_off.mp3" -- Loop end/tail sound, unsilenced
...
SWEP.Primary.Range = 520 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
-- VIEWMODEL
SWEP.ViewModel          = "models/weapons/nsn/c_flamethrower.mdl" -- Viewmodel path
...
-- WORLDMODEL
SWEP.WorldModel         = "models/weapons/nsn/w_flamethrower.mdl" -- Weapon world model path
...
-- [[SPRINTING]] --
SWEP.RunSightsPos = Vector(-1.601, 0, 2.24) -- Change this, using SWEP Creation Kit preferably
SWEP.RunSightsAng = Vector(-11.514, 0, 0) -- Change this, using SWEP Creation Kit preferably
-- [[CROUCHING]] --
...
SWEP.ShootAnimation = {
	["in"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, -- Sequence or act
		["value"] = "shoot", -- Number for act, String/Number for sequence
		["value_is"] = "shoot", -- Number for act, String/Number for sequence
		["transition"] = true
	}, -- Looping Start, fallbacks to loop
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, -- Sequence or act
		["value"] = "nil", -- Number for act, String/Number for sequence,
		["value_is"] = "nil", -- Number for act, String/Number for sequence,
		["is_idle"] = false,
	}, -- Looping Animation
	["out"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, -- Sequence or act
		["value"] = "release", -- Number for act, String/Number for sequence
		["value_is"] = "release", -- Number for act, String/Number for sequence
		["transition"] = true
	}, -- Looping End
}
...
function SWEP:ShootBullet(dmg,num,cone,type,force,tracer)
	return false
end
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		local distSqr = self.Owner:GetPos():DistToSqr(tr.HitPos)

		if distSqr < 520 * 520 then
			...
			local Ignite = function()
				...
				if tr.HitWorld then
					local stuff = ents.FindInSphere(tr.HitPos, 100)

					for k,v in pairs(stuff) do
						if v != self.Owner then
							if v:GetPhysicsObject():IsValid() && !v:IsNPC() && !v:IsPlayer() then
								if !v:IsOnFire() then
									v:Ignite(math.random(4,15), 50)
								end
							end
							if v:IsPlayer() then
				...
				if tr.Entity:IsValid() then
					if tr.Entity:GetPhysicsObject():IsValid() && !tr.Entity:IsNPC() && !tr.Entity:IsPlayer() then
						if !tr.Entity:IsOnFire() then
							tr.Entity:Ignite(math.random(4,15), 50)
						end
					end
					...
					if tr.Entity:IsNPC() then
						if tr.Entity:GetPhysicsObject():IsValid() then
							local npc = tr.Entity:GetClass()
							if npcs[npc] then
								tr.Entity:Fire("Ignite","",1)
							end
							tr.Entity:Ignite(math.random(12,16), 50)
						end
					end
				end
				...
				if SERVER then
					...
					util.Effect("nsn_flamer_impact",fx,true,true)
				end
			end

			timer.Simple(distSqr/(1520*1520), Ignite)
