...
SWEP.PrintName              = "DC-15S"       -- Weapon name (Shown on HUD)
...
-- [[WEAPON HANDLING]] --
SWEP.Primary.Sound = Sound("weapons/nsn/dc15_shoot.mp3") -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.Sound_DryFire          = Sound("weapons/tfa/lowammo.wav")
...
SWEP.Primary.SpreadMultiplierMax = 2.6 -- How far the spread can expand when you shoot. Example val: 2.5
SWEP.Primary.SpreadIncrement = 0.5 -- What percentage of the modifier is added on, per shot.  Example val: 1/3.5
SWEP.Primary.SpreadRecovery = 1.2 -- How much the spread recovers, per second. Example val: 3

-- Range Related

-- DEPRECATED. Automatically converted to RangeFalloffLUT table
SWEP.Primary.Range = 48000/3 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.CrouchAccuracyMultiplier = 0.01 -- Less is more.  Accuracy * 0.5 = Twice as accurate, Accuracy * 0.1 = Ten times as accurate
...
-- VIEWMODEL
SWEP.ViewModel          = "models/weapons/nsn/v_dc15s_nsn.mdl" -- Viewmodel path
...
-- WORLDMODEL
SWEP.WorldModel         = "models/weapons/nsn/w_dc15s_nsn.mdl" -- Weapon world model path
...
-- [[SPRINTING]] --
SWEP.RunSightsPos = Vector(19.9,0,2.52) -- Change this, using SWEP Creation Kit preferably
SWEP.RunSightsAng = Vector(-17.6,45,0) -- Change this, using SWEP Creation Kit preferably
...
SWEP.IronSightsPos = Vector(-4.72, -4.422, 0.959)
SWEP.IronSightsAng = Vector(3, 0, 0)
-- [[INSPECTION]] --
SWEP.InspectPos = Vector(8, -8, -2)
...
SWEP.MuzzleFlashEffect = "rw_sw_muzzleflash_blue" -- Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
-- Tracer Stuff
SWEP.TracerName         = "rw_sw_laser_blue"   -- Change to a string of your tracer name.  Can be custom. There is a nice example at https://github.com/garrynewman/garrysmod/blob/master/garrysmod/gamemodes/base/entities/effects/tooltracer.lua
...
function SWEP:DrawHands()
	...
	if not IsValid(self.Owner.SWHands) then
		self.Owner.SWHands = ClientsideModel("models/weapons/c_clonearms_nsn.mdl")
	...
	elseif self.Owner.SWHands:GetParent() ~= vm then
		self.Owner.SWHands:SetModel("models/weapons/c_clonearms_nsn.mdl")
		self.Owner.SWHands:SetParent(vm)
		self.Owner.SWHands:SetPos(self.Owner:GetShootPos())
		self.Owner.SWHands:SetAngles(self.Owner:EyeAngles())
		self.Owner.SWHands:AddEffects(EF_BONEMERGE)
	elseif self.Owner.SWHands:GetModel() ~= "models/weapons/c_clonearms_nsn.mdl" then
		self.Owner.SWHands:SetModel("models/weapons/c_clonearms_nsn.mdl")
	end
